...
public class PacketSetTransferMode implements IMessage
{
    ...
    public static class HandlerPacketSetTransferMode implements IMessageHandler<PacketSetTransferMode, IMessage>
    {
        ...
        {
            ((TileEntityResistor)ctx.getServerHandler().playerEntity.worldObj.getTileEntity(new BlockPos(message.x, message.y, message.z))).setTransferOncePerTick(message.once);
            return null;
        }
    }
}
